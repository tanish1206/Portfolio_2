...
export const HeroSection: React.FC = () => {
  ...
  return (
    <section className="relative flex min-h-screen w-full flex-col items-center justify-between px-4 py-8 selection:bg-accent-crimson selection:text-white overflow-hidden bg-[#050505] text-white">
      ...
      <div className="pointer-events-none absolute top-0 left-1/2 -translate-x-1/2 h-[650px] w-[900px] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-red-600/35 via-red-950/15 to-transparent blur-2xl z-0" />

      {/* 2. Top-Left Corner Header (Hidden below sm: 640px) */}
      <motion.div
        initial={{ opacity: 0, x: -20 }}
        animate={{ opacity: isTransitioning ? 0 : 1, x: 0 }}
        transition={{ duration: 1, delay: 0.2 }}
        className="hidden sm:flex absolute top-8 left-8 z-20 flex-col space-y-1 font-mono text-[10px] tracking-widest text-white/70 uppercase"
      >
      ...
      {/* 3. Top-Right Corner Header (Hidden below sm: 640px) */}
      <motion.div
        initial={{ opacity: 0, x: 20 }}
        animate={{ opacity: isTransitioning ? 0 : 1, x: 0 }}
        transition={{ duration: 1, delay: 0.2 }}
        className="hidden sm:flex absolute top-8 right-8 z-20 flex-col items-end space-y-1 font-mono text-right text-[10px] tracking-widest text-white/70 uppercase"
      >
      ...
      {/* 4. Left Margin Vertical Label (Hidden below lg: 1024px) */}
      <motion.div
        initial={{ opacity: 0 }}
      ...
      {/* 5. Right Margin Vertical Label (Hidden below lg: 1024px) */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: isTransitioning ? 0 : 1 }}
        transition={{ duration: 1, delay: 0.4 }}
      ...
      <div className="hidden md:block pointer-events-none absolute left-20 top-1/2 -translate-y-1/2 z-10 font-mono text-xs text-red-600/50">
        +
      </div>
      <div className="hidden md:block pointer-events-none absolute right-20 top-1/2 -translate-y-1/2 z-10 font-mono text-xs text-red-600/50">
        +
      </div>
      ...
      <div className="relative z-10 my-auto flex flex-col items-center justify-center w-full max-w-5xl">
        ...
        <motion.div
          ...
          className="-mt-10 sm:-mt-14 md:-mt-20 z-20 flex items-center justify-center space-x-3 sm:space-x-6 md:space-x-8 font-space text-3xl sm:text-6xl md:text-7xl lg:text-8xl font-light tracking-[0.2em] sm:tracking-[0.35em] md:tracking-[0.4em]"
        >
          <span className="text-white">TANISH</span>
          <span className="text-red-600 drop-shadow-[0_0_25px_rgba(200,40,31,0.5)]">SONI</span>
        </motion.div>
        ...
        <motion.div
          ...
          className="mt-5 flex flex-col items-center space-y-3 w-full max-w-xl px-4"
        >
      ...
      >
        <div className="font-mono text-[10px] md:text-[11px] tracking-widest text-red-500/90 uppercase flex items-center space-x-1">
          ...
          <span className="text-red-600 font-semibold animate-pulse">SCROLL TO BEGIN</span>
          <span className="text-white/40">]</span>
        </div>
        <motion.div
          animate={{ y: [0, 6, 0] }}
